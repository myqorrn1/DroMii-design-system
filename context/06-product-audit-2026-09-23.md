...
실측 자료다. Core 값의 채택·폐기 결정은 이 문서에서 하지 않는다. 단계 3에서
큰 파운데이션 차이만 별도 문서 없이 보고하고 소유자 승인을 받는다.
...
- **두 제품의 공통 후보:** 상단 제품·계정 영역, 좌측 업무 탐색, 표·검색·페이지 이동,
  ...
  제품 맥락으로 남겨야 한다. Core의 구체적인 유지·변경 경계는 제품 적용 계획에서 매핑한다.
...
1. 제품 실측값·현행 Core·KRDS의 차이가 큰 파운데이션 항목만 결정 요청으로 보고한다.
   제품별 세부 **유지 / 변경 / 폐기** 매핑은 실제 적용 계획에서 한다.
2. K-AQUAS의 11–14px, D-ROAD의 13–16px·`vw`, 현재 Core 13px을 함께 놓고
   본문 크기와 `default / compact` 선택지를 제시한다.
3. D-ROAD 다크 기본값과 고대비 명도 축의 자리를 선택지로 정리한다. 인증 화면의
   주황과 업무 화면의 보라를 같은 역할로 취급하지 않는다.
4. K-AQUAS의 30px 지도 제어, D-ROAD의 40·44px 제어와 현행 Core 높이의
   연결을 결정한다. K-AQUAS z-index 테마와 D-ROAD CSS·MUI 혼용의 실제 연결
   순서는 적용 계획에서 다룬다. 이 단계에서 제품 코드는 수정하지 않는다.
