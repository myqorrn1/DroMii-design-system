...
> **최신 제품 코드 감사:** [2026-09-23 제품 소스 재감사](06-product-audit-2026-09-23.md)를
> 먼저 읽을 것. [운영 프론트 확인](05-live-frontends-2026-09-23.md)은 빌드 계통의 근거다.
> 아래 수치와 Vite 관련 서술은 당시 스냅샷·후보 소스의 기록이며 현재 운영 D-ROAD의
> 프론트 근거로 직접 사용하지 않는다.
> K-AQUAS도 이후 `src` 20개 파일이 바뀌었고 MUI z-index 테마가 추가됐다.
> 아래 수치를 현행 빈도나 확정 Core 값으로 쓰지 않는다.

> K-AQUAS `src/` **28,735줄** + D-ROAD `src/` **8,033줄** 전수 스캔 (2026-08-28)
> `.js .jsx .css` 정규식 스캔. 3자리 hex와 `rgb()`는 6자리로 정규화해 집계.
> D-FIND는 코드가 없어 측정 제외.

## 2026-09-11 후보 소스 재감사 기록

위 수치는 2026-08-28 스냅샷을 설명하는 역사적 기준입니다. 현재 K-AQUAS 로컬 저장소와
서버의 D-ROAD 후보 소스(`/home/dromii/dromii_V5_vite`)를 다시 확인한 기록입니다.
2026-09-23 운영 페이지는 다른 React 빌드로 확인되었으므로 아래 D-ROAD 판단은
최신 운영 소스에 비춰 재검토해야 합니다.
...
- 당시 살핀 Vite 후보에는 `#272934` 기반 다크 인증 화면, 밝은 관리자 화면, Vite 기본 CSS가
  함께 남아 있었습니다. 운영 프론트의 기준으로 승격하지 않습니다.
- 당시 후보의 공용 셸 구조는 `components/header/Header.jsx`,
  `components/siderbar/Sidebar.jsx`, `components/layout/Layout.jsx`입니다.

따라서 아래의 파일 수와 빈도는 값의 출처를 설명할 때만 쓰고, 컴포넌트의 포함 여부는
2026-09-23 운영 기준 재확인과 실제 최신 코드로 판단합니다.
...
## 1. 두 제품의 기술 스택 — 8월 28일 기록, 현재는 재감사 참조

| | K-AQUAS | D-ROAD |
|---|---|---|
| React / MUI | 18.3 / 5.15 | 19.1 / 7.2 |
| 스타일 방식 | styled-components + MUI `sx` + Bootstrap 전역 CSS | 컴포넌트별 순수 CSS 30개 파일 |
| 테마 레이어 | 당시 **없음** → 현재 z-index 전용 `createTheme`·`ThemeProvider` 있음 | 현재 전역 `createTheme` 없음 |
| 기본 명도 | 라이트 | 다크 |
