...
## 2026-09-11 서버 재감사 정정

위 수치는 2026-08-28 스냅샷을 설명하는 역사적 기준입니다. 현재 K-AQUAS 로컬 저장소와
서버의 D-ROAD 후보 소스(`/home/dromii/dromii_V5_vite`)를 다시 확인한 결과, 기능 포함 여부는
다음 사실을 우선합니다.

- K-AQUAS 사용자 관리에는 MUI `TablePagination`이 실제로 사용됩니다.
- D-ROAD 사용자 관리와 접속 로그에는 서버 페이지네이션이 있고, 접속 로그에는 체크박스
  선택과 선택 항목 처리가 있습니다.
- 양쪽 모두 앱 셸, 검색·필터 폼, 상태 배지, 관리형 표, 업로드 폼과 확인 흐름이 반복됩니다.
- 현재 D-ROAD 소스에는 `#272934` 기반 다크 인증 화면, 밝은 관리자 화면, Vite 기본 CSS가
  함께 남아 있습니다. 저장소에 있다는 이유만으로 모두 제품 기준으로 승격하지 않습니다.
- D-ROAD의 현재 공용 셸 후보는 `components/header/Header.jsx`,
  `components/siderbar/Sidebar.jsx`, `components/layout/Layout.jsx`입니다.

따라서 아래의 파일 수와 빈도는 값의 출처를 설명할 때만 쓰고, 컴포넌트의 포함 여부는
이 정정과 실제 최신 코드로 판단합니다.
...
**양쪽 다 공용 테마 레이어가 없습니다.** 제품 코드 위에 토큰만 덧씌우는 것으로 끝내지 않고,
공통 구조와 동작을 Core 컴포넌트로 분리해야 합니다.
