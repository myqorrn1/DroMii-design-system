...
> K-AQUAS와 D-ROAD의 코드에서 공통 구조를 추출한 **공공기관 실무용 Core 검토본**입니다.
> 공용 토큰, HTML·CSS 컴포넌트 견본, Markdown 사용 기준을 하나의 코드 기준으로 관리합니다.
>
> - [Codex·Claude Code 작업 인계](docs/HANDOFF.md) — 현재 방향, 작업 방법, 다음 순서
> - [공공업무 디자인 방향](docs/PUBLIC_SECTOR_DIRECTION.md) — KRDS 참고 범위와 유지·수정할 것
...
2026-09-23부터 대상 사용자를 **공공기관의 내부 관리자·수질 분석·도로 조사 담당자**로
명시합니다. KRDS를 공용 컴포넌트의 기본 설계 기준으로 삼되 이 제품을
공식 정부 누리집이라고 가정하지 않습니다. 정부 공식 배너나 기관 표식은 운영 주체와
납품 요구사항이 확인되기 전까지 사용하지 않습니다. 기존 제품 로고와 기준색은 유지합니다.
...
docs/              상태 · 토큰 · 컴포넌트 · 패턴 · 적용 가이드
  ...
  PUBLIC_SECTOR_DIRECTION.md  공공기관 실무용 설계 방향과 KRDS 적용 경계
...
- 실제 브라우저 200% 확대와 헤더·메뉴·폼의 세부 키보드를 검증
- 선택형 로그와 검색·필터의 서버 실패·부분 성공·권한 상태 설계
- 긴 표·위험 동작의 공통 규칙 완성
- 제품별 밀도와 업로드·지도 도구의 상세 동작 확정
- 디자인 확정 후 대표 화면을 새로 설계
- 이후 React 공용 패키지, D-FIND 설정과 실제 제품 적용을 검토
...
- KRDS를 참고하되 드로미 솔루션을 공식 정부 누리집으로 가정하지 않는 적용 경계 기록
...
**KRDS(대한민국 정부 디자인시스템)** 를 기준으로 다시 맞췄습니다.
