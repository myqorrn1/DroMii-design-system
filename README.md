...
> - [공통 시각 스타일](docs/VISUAL_STYLE.md)
...
우선 사용자는 **공공기관의 내부 관리자·수질 분석·도로 조사 담당자**입니다. 하지만
Core의 출발점은 KRDS가 아니라 **K-AQUAS와 D-ROAD의 제품 코드**입니다. 기존 기준색,
제공 로고, 메뉴 구조와 익숙한 흐름을 유지하면서 접근성·불일치·임시방편을 고칩니다.
판단 순서는 **제품 근거 → Core 공통화 가치 → KRDS·접근성 점검**입니다. KRDS 채택 범위와
...
background: var(--dm-accent-500);      /* X — 색상 단계 직접 참조. K-AQUAS는 500, D-ROAD는 400이 기준 */
```
...
docs/              상태 · 토큰 · 컴포넌트 · 패턴 · 적용 가이드
  ...
  HANDOFF.md         현재 방향·작업 절차·다음 순서의 인계 기준
  PUBLIC_SECTOR_DIRECTION.md  KRDS 채택 범위·예외와 판단 순서
  FORM_LAYOUT.md     폼 구조·오류·저장 상태 사용 규칙
  DATA_TABLE.md      표의 열·행 동작·선택형 목록 적용 경계
  LIST_FILTER.md     검색 조건·결과·선택 초기화 기준
  VISUAL_STYLE.md    공통 시각 스타일과 헤더·메뉴 기준
  TOKENS.md          토큰 구조와 변경 방법
  COMPONENTS.md      컴포넌트 범위와 상태
  PATTERNS.md        업무 조합 패턴의 설계 기준과 보류 범위
  THEMING.md         Core와 제품 확장의 경계
  ACCESSIBILITY.md   접근성 기준과 검사
  APPLICATION.md     디자인 확정 뒤 적용 절차

context/           판단 재료 — 값을 바꾸기 전에 읽을 것
  01-as-is.md        2026-08-28 코드 실측 — 현재 값으로 인용하기 전에 06 참조
  02-keep.md         그대로 쓸 것 / 고칠 것, 그리고 그 판단 기준
  03-rules.md        시스템 규칙 14개
  04-terms.md        용어 사전 — 새 문서를 쓰기 전에 확인
  05-live-frontends-2026-09-23.md  운영 빌드 계통 확인
  06-product-audit-2026-09-23.md   최신 제품 코드 재감사
```

## 고치기

순서가 있습니다. **`tokens/source.json` → `npm run tokens:build` → `components/base.css` → 견본 페이지.**
...
1. **실측 근거가 있는가** — 최신 `context/06-product-audit-2026-09-23.md`와 제품 코드에
   있는 값인가, 아니면 새로 만드는 것인가. `context/01-as-is.md`는 역사적 스냅샷이다.
2. **역할 토큰인가** — 컴포넌트가 색상 단계(`--dm-accent-500`)를 직접 보면 제품을 하나 더 붙일 때 깨집니다
...
공통 구조와 행동 위에 **브랜드 · 명도 · 밀도** 세 축을 조합합니다. 제품 자산은 정해진 자리에
주입하며, 로고 분석은 이번 구축 범위에서 제외합니다.
...
1. [두 제품의 소스 재감사](context/06-product-audit-2026-09-23.md)를 근거로 제품 요소와
   Core의 마이그레이션 매핑, KRDS 실측 대조를 작성합니다. 본문 글자 크기,
   D-ROAD 다크 기본값 등 큰 변경은 선택지를 제시하고 승인을 기다립니다.
...
- 제품 코드 추출과 리팩토링을 최우선으로 복원하고 KRDS를 세 번째 점검 기준으로 재배치
- KRDS 채택·제외·보류 항목과 승인 지점을 한 문서에 모음
- 제품 감사 → 마이그레이션 매핑·대조 → 승인 → 토큰 → 승인 → 업무 패턴 순서로 인계

**2026-09-23 — 선택형 로그와 좁은 유효 화면 보강**

- D-ROAD 로그 작업을 바탕으로 가상 데이터의 검색·선택·삭제 확인 견본 추가
- 현재 표시된 항목만 전체 선택, 혼합 상태, 선택 건수와 삭제 취소·결과 동작 확인
- 512px 유효 화면에서 앱 셸을 세로 재배치하고 표를 내부 가로 스크롤로 전환
- 실제 브라우저 200% 확대, 서버 오류·권한·부분 성공은 미검증으로 기록

**2026-09-23 — 공공기관 실무용 Core 방향 전환**

- KRDS 참고 범위의 첫 기록. 현재 범위는 위의 2026-09-23 방향 정정을 따른다.
- 폼 1열 기본형과 관련 필드 2열 예외, 앱 셸의 본문 건너뛰기 추가
- 일괄 작업이 없는 사용자 표에서 선택 열 제거, 날짜 형식과 상세 버튼의 대상 명확화
- 1024·1200·1440·1920px의 폼·메뉴·표 배치와 본문 건너뛰기 키보드 동작 검증
- Codex·Claude Code 공통 인계를 위한 `HANDOFF.md`, `AGENTS.md`, `CLAUDE.md` 추가

**2026-09-23** — 대표 화면 시안 보류

- 대시보드, 사용자 관리, 데이터 업로드, 지도 워크스페이스의 대표 화면 시안을 제거
- 공용 토큰과 개별 컴포넌트 견본은 유지하고, Core 디자인 확정 뒤 대표 화면을 다시 제작
- 운영 D-ROAD는 이전 감사의 Vite 후보가 아닌 React 빌드임을 확인하고 헤더·메뉴 스타일을 재검토

**v0.15 (2026-09-11)** — DroMii Core 첫 통합 검토본

- K-AQUAS와 D-ROAD의 현재 프론트 코드를 다시 확인해 공용 앱 셸, 폼, 탭, 툴팁,
  페이지네이션, 차트 팔레트, 아이콘과 화면 폭 기준 추가
- 대시보드, 사용자 관리, 데이터 업로드, 지도 워크스페이스 조합 화면 추가
- K-AQUAS `TablePagination`과 D-ROAD 선택형 로그 표를 근거로 표 선택과 페이지네이션 복원
- 당시 검토한 D-ROAD Vite 후보의 기본값과 임시 스타일은 Core 근거에서 제외
- 토큰 194개, 테마별 CSS 선언 260개로 확장

**v0.14 (2026-09-10)** — 토큰 원본과 자동 검사

- 182개 고유 토큰, 242개 테마별 선언을 `tokens/source.json` 원본으로 이전
- `npm run tokens:build`로 기존 CSS 변수와 선택자를 같은 값으로 생성
- JSON/CSS 동기화, 별칭 누락·순환, 미정의 CSS 참조, HTML 링크·라벨, 주요 대비 자동 검사
- 외부 런타임 의존성 없이 Node.js만으로 생성과 검사를 수행

**v0.13 (2026-09-10)** — 공용 시스템 기준선

- 관리 기준을 코드 중심으로 확정하고 프로젝트 범위와 로드맵, 결정 기록을 추가
- 제품별 테마 한 축을 브랜드 · 명도 · 밀도 세 축으로 분리할 방향 확정
- K-AQUAS 채움 버튼의 상태별 글자색을 분리해 기본 5.96:1, hover 4.76:1, pressed 7.31:1 확보
- 플레이스홀더 역할 토큰 추가: 라이트 6.24:1, 다크 6.39:1
- 입력 견본의 시각 라벨을 실제 `label`로 바꾸고 도움말·오류 문구를 입력 요소와 연결
- 다이얼로그 radius를 문서 규칙과 같은 10px로 수정
- v0.9에서 제거한 정렬 · 일괄 선택 · 스켈레톤 · 실행취소 관련 현재 문서 정리

**v0.12 (2026-09-02)** — 한국어 용어 정리
문서 전체의 한국어가 영어 개념을 그때그때 옮긴 말이라 국내 현장에서 통하지 않았습니다.
당시 접근성 점검에 **KRDS(대한민국 정부 디자인시스템)** 를 대입했습니다. 현재의 판단
순서는 위의 2026-09-23 방향 정정을 따릅니다.

- `context/04-terms.md` 신설 — 용어 사전. 색상 · 형태 · 요소 · 서술어 표와
  영문을 그대로 둘 목록. **새 페이지를 만들 때 여기 없는 말을 지어내지 않는다**
- 명사 — 면 → 배경/표면, 램프 → 색상 단계, 액센트 → 강조 색상,
  틴트 → 옅은 색, 스크림 → 딤드, 깊이 → 엘리베이션, 시맨틱 → 시멘틱
- 서술어 — 회피책 → 임시방편, 새어 들어온 → 섞여 들어온, 발견값 → 코드에서 찾은 값 등 21곳
- 얹다 · 파여 보이다 · 흡수하다처럼 **이미 자연스러운 말은 바꾸지 않았습니다**
- 버전 표기 통일 (index · overview · tokens.css · README가 서로 달랐음)

**v0.11 (2026-09-02)** — 외부 리뷰 반영
- danger 역할 토큰 4개 — 버튼이 색상 단계를 직접 참조하던 것을 역할 토큰으로
- `surface-disabled` · K-AQUAS `surface-raised` — 없어서 다른 토큰을 대신 쓰던 자리
- **포커스 링 accent-500 → accent-600** — 2.98:1은 WCAG 1.4.11(비텍스트 3:1) 미달.
  포커스 링은 브랜드 요소가 아니라 접근성 장치라 예외 대상이 아니다
- 다크 `text-disabled` → neutral-600 — 400은 muted보다 밝아 위계가 뒤집혔다
- `xs` 24px 한 벌 추가 — 지도 위 아이콘 버튼(14회)이 토큰 없이 쓰이고 있었다
- 규칙 3의 적용 범위를 시멘틱 색으로 좁히고, 4-B(그리드 예외)를 새로 씀
...
- 토큰 2개 — `shadow-raised` · `overlay-scrim`. 그림자 단계는 있었으나 역할 토큰이 없어
  컴포넌트가 색상 단계를 직접 참조해야 했음. 딤드는 라이트 .44 · 다크 .62로 나눔
...
- `--dm-surface-shimmer` 신설 — 스켈레톤이 행 상태 토큰을 대신 쓰던 것을 분리
...
- 규칙 3의 적용 범위를 시멘틱 색으로 한정 (브랜드 강조 색상는 예외)
