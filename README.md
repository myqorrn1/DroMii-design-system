...
1. 운영 D-ROAD React 소스와 로컬 K-AQUAS 프런트를 같은 항목으로 재감사합니다.
2. 제품 요소와 Core의 마이그레이션 매핑, KRDS 실측 대조를 작성합니다. 본문 글자 크기,
   D-ROAD 다크 기본값 등 큰 변경은 선택지를 제시하고 승인을 기다립니다.
3. 승인된 값으로 토큰과 테마 축을 재구성하고 전후 화면을 검증합니다.
4. 다시 승인된 뒤 표·검색, 위험 동작, 업로드, 지도 패턴을 순서대로 설계합니다.
